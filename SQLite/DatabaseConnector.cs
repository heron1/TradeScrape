...
namespace TradeScraper.SQLite
{
	public class DatabaseConnector
	{
		...
		public DatabaseConnector()
		{
			...
			_supportedPlatforms = new List<string>
			{
				"bitfinex", "kucoin"
			};
		}
		...
		public (bool success, string apiKey, string secretKey, string passphrase) GetCredentials(string platform)
		{
			...
			string query = $"SELECT * FROM credentials WHERE platform = '{platform}'";
			...
			//returns true if credentials sucessfullly retrieved, along with the credentials cols
		{
			...
			string query = "SELECT default_platform FROM meta";
			string defaultPlatform = "";
			var reader = ExecuteReader(query);
			while (reader.Read())
			{
				defaultPlatform = reader["default_platform"].ToString();
			}
			if (reader.StepCount != 1)
				return (false, "", "", "", "");
		...
		public bool SetCredentials(string platform, string apiKey, string secretKey, string passphrase) //returns true if successful
		{
			string query = $"UPDATE credentials SET apiKey = '{apiKey}', secretKey = '{secretKey}', passphrase = '{passphrase}' " +
		...
		{
			string query = $"UPDATE meta SET default_platform = '{platform}';";
			return (ExecuteNonQuery(query) == 1);
		}
		...
		{
			string query = "SELECT remove_keys_on_exit FROM meta;";
			var reader = ExecuteReader(query);
			reader.Read();
			int val = Convert.ToInt32(reader["remove_keys_on_exit"].ToString());
		...
		{
			string query = "UPDATE credentials SET apiKey = null, secretKey = null;";
			ExecuteNonQuery(query);
		}

		public bool EnsureDatabaseIntegrity() //returns true if integrity test passed, adds missing platforms
		{
		...
		private bool InsertMissingPlatforms() //returns true if operation successful, false otherwise
		{
			...
			while (reader.Read())
			{
				foundPlatforms.Add(reader["platform"].ToString());
			}
			foreach (var platform in _supportedPlatforms)
			{
				if (!foundPlatforms.Contains(platform))
				{
					string query = $"INSERT INTO credentials VALUES ('{platform}', null, null, null)";
		...
		public static void PrintReader(SQLiteDataReader reader)
		{
			if (reader.HasRows)
			{
				...
				while (reader.Read())
				{
					foreach (var col in reader.GetValues())
					{
						str.Append($"{col.ToString()}: {reader[col.ToString()]} | ");
					}
